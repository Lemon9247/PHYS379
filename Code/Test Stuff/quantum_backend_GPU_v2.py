...
class Grover:
	...
	def __init__(self,oracle_function,bits,verbose=None):
		...
		diffuser = self.compute_diffuser()
		quantum_oracle = self.compute_oracle(oracle_function)
		cp.cuda.stream.get_current_stream().synchronize()
		del self.hadamards
		cp.cuda.stream.get_current_stream().synchronize()
		self.cirucit = quantum_oracle*diffuser
	...
	def compute_diffuser(self):
		...
		diffuser_sparse = cpx.csc_matrix(diffuser)
		del diffuser

		if self.verbose: print("Done!")
		return diffuser_sparse

	def compute_oracle(self,oracle_function):
		if self.verbose: print("Computing Quantum Oracle...")
		quantum_oracle_diagonal = np.ones(2**self.bitnumber)
		for i in range(2**self.bitnumber):	# Construct oracle on CPU as looping over elements on the GPU is slow
			try:	# Use try/except in case the number of bits exceeds original register bitlength
				if oracle_function(i):
					quantum_oracle_diagonal[i] = -1
			except:
				continue
		quantum_oracle = sp.dia_matrix(quantum_oracle_diagonal)
		oracle_sparse = cpx.csc_matrix(quantum_oracle)

		if self.verbose: print("Done!")
		return oracle_sparse
