...
def adaptive_search(database,max_val, min_val):
	...
	m = 1
	fails = 0
	if fails < max_val and fails > min_val:
		iterations = random.randint(1,np.ceil(m))
		J = quantum.Grover(lambda x: adaptive_oracle2(x,x_0,database),bits)
		q = J.search(iterations,errorp=0.9) #this is here where I change the errorp, try putting errorp=0
...
def multi_trial_durr_hoyer(shots,trials,database,max_val,min_val):
	outputs = [[] for i in range(trials)]
	bits = int(np.ceil(np.log2(len(database))))
	for j in range(trials):
		for i in range(shots):
			t = adaptive_search(database,max_val,min_val)
			outputs[j].append(t)
...
def main1(shots,trials,bits):
	max_val = 9 #here is where i change the range of values... if you equal max_val and min_val it should work as
	min_val = 1 #the threshold beofre worked, but it doesn't
